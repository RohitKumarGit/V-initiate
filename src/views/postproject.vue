<template>
    <div class="container postproject">
        <div class="header">
            Post a project
        </div>
          <b-field label="Enter Title" class="mt-4">
            <b-input v-model="title"></b-input>
        </b-field>
        <b-field label="Tell something about the project.">
            <b-input  type="textarea" v-model="desc"></b-input>
        </b-field>
        <b-field label="Select datetime" style="width:40%">
            <b-datetimepicker
            v-model="date"
                rounded
         ...
         <b-field label="Select State & city" style="width:40%" class="has-display-flex">
      
        <b-autocomplete
                rounded
                @select="op=>selectedState = op "
                v-model="state"
               
                :data="selectStateFilter"
         ...
         <b-field label="Give Full address">
            <b-input maxlength="100" type="textarea" v-model="address"></b-input>
        </b-field>
        ...
        <b-upload v-model="file" class="file-label">
            ...
            <span v-if="file">uploaded {{pgrs}} %</span>
        </b-upload>
       
    </b-field>
     <b-button type="is-info" @click="publish">Publish</b-button>
    </div>
</template>
<script>
var s_a = new Array();
...
export default {
    data(){
        return {
            ...
            date:null,
            address:"",
            state_names :["Andaman & Nicobar", "Andhra Pradesh", "Arunachal Pradesh", "Assam", "Bihar", "Chandigarh", "Chhattisgarh", "Dadra & Nagar Haveli", "Daman & Diu", "Delhi", "Goa", "Gujarat", "Haryana", "Himachal Pradesh", "Jammu & Kashmir", "Jharkhand", "Karnataka", "Kerala", "Lakshadweep", "Madhya Pradesh", "Maharashtra", "Manipur", "Meghalaya", "Mizoram", "Nagaland", "Orissa", "Pondicherry", "Punjab", "Rajasthan", "Sikkim", "Tamil Nadu", "Tripura", "Uttar Pradesh", "Uttaranchal", "West Bengal"]
        }
    },
    methods:{
        async publish(){
            ...
            var imagesRef = storageRef.child(name+'.jpg');
            const upload = imagesRef.put(this.file)
            upload.on("state_changed",(snapshot)=>{
                    this.pgrs = Math.floor((snapshot.bytesTransferred / snapshot.totalBytes) * 100);
            },(error)=>{ alert(error.message)},()=>{
                    upload.snapshot.ref.getDownloadURL().then(async (downloadURL) => {
                      const datas = {
                title:this.title,
                desc:this.desc,
                when : (new Date(this.date)).toISOString(),
                user:this.user._id,
                city:this.city.trim(),
            ...
            console.log(datas)
            try {
                const {data} = await axios.post('/api/project',datas);
                this.$router.push('/view_project/'+data.project._id)
            } catch (error) {
                alert(error.message)
            }
                        });
    ...
    computed:{
        ...
        states(){
            var k = 1;
            var states =  this.state_names.map(v=>{
                    k+=1;
                    return {val:v,key:k-1};
            })
            return states;
        },
        cities(){
            return s_a[this.selectedState.key].split(" | ")
        },
        selectStateFilter(){
            return this.states.filter(option=>{
                return (
                    option.val.toLowerCase().indexOf(this.state.toLowerCase()) >=0
                )
            })
        },
        selectCityFilter(){
            ...
            return this.cities.filter(v=>{
                return (
                    v.toLowerCase().indexOf(this.city.toLowerCase())>=0
                )
            })
        }
    }
